...
/// Where this copy of PortKilla came from; decides what "update" means.
enum InstallSource: String {
    ...
    static func detect(bundleURL: URL = Bundle.main.bundleURL) -> InstallSource {
        let caskrooms = ["/opt/homebrew/Caskroom/portkilla", "/usr/local/Caskroom/portkilla"]
...
/// The facts a bug report needs, gathered the same way for `portkilla
...
enum Diagnostics {
    ...
    static func report() -> [Line] {
        ...
        return [
            Line(label: "PortKilla", value: UpdateChecker.currentVersion ?? "dev"),
            ...
            Line(label: "portkilla on PATH", value: onPath),
    ...
    /// Which `portkilla` a shell would run, and whether it is this binary.
    private static func pathResolution(of executable: String) -> String {
        let path = ProcessInfo.processInfo.environment["PATH"] ?? ""
        for directory in path.split(separator: ":") {
            let candidate = "\(directory)/portkilla"
