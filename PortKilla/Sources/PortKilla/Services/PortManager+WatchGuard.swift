...
extension PortManager {
    ...
    func firePendingFreeNotifications(with ports: [PortInfo]) {
    ...
    func guardKillTarget(for port: Int, in ports: [PortInfo]) -> PortInfo? {
    ...
    func processWatchedPorts(with ports: [PortInfo], guardOwners: [Int: AgentOwner] = [:]) {
