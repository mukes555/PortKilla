...
public enum PortKillaCLI {
    ...
    private static func freePort(prefer: Int, range: ClosedRange<Int>, json: Bool) -> Int32 {
        ...
        if let port {
            print(port) // just the number, so `PORT=$(portkilla free-port)` works
        } else {
    ...
    private static func schema(for command: String?) -> Int32 {
        guard let text = OutputSchemas.render(command) else {
            printError("portkilla: no schema for '\(command ?? "")'. Known: \(OutputSchemas.commands.joined(separator: ", "))")
            return CLIExit.usage
        }
        print(text)
        return CLIExit.ok
    ...
    - Exit code 3 means the port belongs to another agent's running session, or to
      nobody PortKilla can name (most likely a person's server). The refusal is on
