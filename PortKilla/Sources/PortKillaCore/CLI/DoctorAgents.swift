...
/// `portkilla doctor --agents`: the compatibility matrix checked against
...
public enum DoctorAgents {
    ...
    public static func run(json: Bool) -> Int32 {
        let report = report()
        if json {
            return PortKillaCLI.printJSON(report) ? CLIExit.ok : CLIExit.internalError
        }
        print(text(report))
        return CLIExit.ok
    }
    ...
    static func text(_ report: Report) -> String {
        ...
        if let me = report.caller {
            let how = me.source == .declared ? "declared via PORTKILLA_OWNER" : "from \(me.source.rawValue)"
