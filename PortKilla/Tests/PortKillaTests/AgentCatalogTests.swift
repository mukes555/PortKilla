...
final class AgentCatalogTests: XCTestCase {
    ...
    func testAgentsDocDescribesEveryCatalogEntry() throws {
        ...
        for needle in ["PORTKILLA_OWNER", "PORTKILLA_SESSION", "whois", "--orphaned", "doctor --agents"] {
    ...
    func testDoctorAgentsSeesRunningToolsAndInstalledExecutables() throws {
        ...
        let bin = FileManager.default.temporaryDirectory.appendingPathComponent("portkilla-doctor-\(UUID().uuidString)")
    ...
    func testDeclaredOwnerCarriesADeclaredSession() {
        let owner = AgentAttribution.ownerFromEnvironment(["PORTKILLA_OWNER": "my-bot", "PORTKILLA_SESSION": " run-7 \n"], in: .empty)
        ...
        XCTAssertNil(AgentAttribution.ownerFromEnvironment(["PORTKILLA_OWNER": "my-bot", "PORTKILLA_SESSION": "   "], in: .empty)?.sessionKey)
    }

    func testMarkerOwnerTakesADeclaredSessionWhileTheToolRuns() {
        let running = table([(100, 1, "/opt/homebrew/bin/codex")])
        let live = AgentAttribution.ownerFromEnvironment(["CODEX_SANDBOX": "seatbelt", "PORTKILLA_SESSION": "s2"], in: running)
        XCTAssertEqual(live?.name, "Codex CLI")
        XCTAssertEqual(live?.sessionKey, "s2")
        XCTAssertTrue(live?.isLiveAgentSession == true)

        let ended = AgentAttribution.ownerFromEnvironment(["CODEX_SANDBOX": "seatbelt", "PORTKILLA_SESSION": "s2"], in: .empty)
    ...
    func testTreeCallerPicksUpItsDeclaredSession() {
        let t = table([(100, 1, "claude"), (200, 100, "/bin/zsh"), (300, 200, "portkilla whoami")])
        let caller = AgentAttribution.callerOwner(callerPid: 300, in: t, environment: ["PORTKILLA_SESSION": "s9"])
        XCTAssertEqual(caller?.name, "Claude Code")
        XCTAssertEqual(caller?.sessionPid, 100)
        XCTAssertEqual(caller?.sessionKey, "s9")
        let withOwnId = AgentAttribution.callerOwner(callerPid: 300, in: t, environment: ["PORTKILLA_SESSION": "s9", "CLAUDE_CODE_SESSION_ID": "uuid"])
    ...
    func testEvidencePrefersTheDeclarationAndListsMarkers() {
        ...
        let env = ["CLAUDECODE": "1", "PORTKILLA_OWNER": "my-bot", "PORTKILLA_SESSION": "s1", "HOME": "/Users/me"]
        ...
        XCTAssertEqual(Set(evidence.markers.keys), ["CLAUDECODE", "PORTKILLA_OWNER", "PORTKILLA_SESSION"], "only allowlisted keys are ever shown")
        XCTAssertEqual(evidence.markersLine, "CLAUDECODE=1, PORTKILLA_OWNER=my-bot, PORTKILLA_SESSION=s1")
    }
