...
final class DistributionTests: XCTestCase {
    ...
    func testPerCommandHelpParses() {
        ...
        for topic in ["list", "kill", "free", "wait", "history", "whoami", "doctor"] {
            XCTAssertTrue(CLIArguments.usage(for: topic).contains("portkilla \(topic)"), topic)
        }
    ...
    func testDiagnosticsReportHasTheBugReportFacts() {
        ...
        for expected in ["PortKilla", "macOS", "Architecture", "Install source", "Scanner", "portkilla on PATH", "Launch at login"] {
