...
final class GuardHolesTests: XCTestCase {
    ...
    func testDeclaredOwnersNeverExpireByLiveness() {
        let nobody = table([(300, 1, "node server.js")])
        let declared = AgentAttribution.owner(ofPid: 300, in: nobody) { _ in ["PORTKILLA_OWNER": "my-bot"] }
    ...
    func testMineMatchesWhatKillWouldAllow() {
        ...
        options.mine = true
        XCTAssertEqual(PortKillaCLI.filtered(ports, by: options, caller: me).map(\.port), [1])
        ...
        XCTAssertEqual(PortKillaCLI.filtered([claudePort], by: options, caller: terminal).map(\.port), [],
    ...
    func testAgentDocsMentionWhatAgentsNeed() {
        for needle in ["exit code 3", "stderr", "--dry-run", "PORTKILLA_OWNER", "free", "wait", "history", "--pid"] {
            XCTAssertTrue(PortKillaCLI.agentDocs.lowercased().contains(needle.lowercased()), "agent-docs should mention \(needle)")
        }
    }
}
