...
final class ReviewFixesTests: XCTestCase {
    ...
    override func setUp() {
        suite = "com.mukes555.PortKilla.tests.\(UUID().uuidString)"
    ...
    func testProjectsThatStoppedListeningLeaveTheConfigCache() throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("portkilla-config-\(UUID().uuidString)")
    ...
    func testFreePortCanSkipTheBindProbe() {
        XCTAssertEqual(PortKillaCLI.firstFreePort(prefer: 3000, range: 3000...3002, listening: [3000], probe: false), 3001)
        XCTAssertNil(PortKillaCLI.firstFreePort(prefer: 3000, range: 3000...3001, listening: [3000, 3001], probe: false))
    }
    ...
    func testTheInstallerRefusesGarbledMarkersAndUnreadableFiles() throws {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("portkilla-docs-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
    ...
    func testDoctorFindsToolsWhereverTheLocatorLooks() throws {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("portkilla-doctor-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
