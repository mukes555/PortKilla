...
struct KillFlow {
    ...
    func requestManagedStop(_ port: PortInfo, managed: ManagedRuntime, warnings: [String] = []) -> Bool {
        switch managed.kind {
        case .reloader:
            let lines = ["It supervises '\(port.processName)' (PID \(port.pid)) on :\(port.port) and \(managed.consequence). PortNanny stops the supervisor and everything under it."] + warnings
    ...
    private func runManagedChoice(_ port: PortInfo, managed: ManagedRuntime, warnings: [String]) -> ManagedChoice {
        ...
        if let arguments = managed.stopArguments, let command = managed.stopCommand {
            lines = ["A plain kill would not last: \(managed.consequence).", "PortNanny can run `\(command)` instead."]
