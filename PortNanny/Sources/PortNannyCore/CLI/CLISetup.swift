...
/// `portnanny setup`: the steps that make each AI tool on this Mac free
/// ports through PortNanny, offered one at a time. Every change is asked
...
public enum CLISetup {
    ...
    public static func run(_ options: CLICommand.SetupOptions) -> Int32 {
        ...
        print("PortNanny setup for \(project.path)\n")
        ...
        for step in steps {
            ...
            case .writeRules, .runCommand:
                ...
                if outcome.ok { print("  " + outcome.message) } else {
                    PortNannyCLI.printError("  " + outcome.message)
                    failures += 1
                }
            }
            print("")
        }
        print("Run `portnanny doctor --agents` any time to see how each tool is recognised.")
    ...
    public static func plan(agents report: DoctorAgents.Report, project: URL, pathHint: String? = nil,
        ...
        let onPath = pathHint ?? Diagnostics.report().first { $0.label == "portnanny on PATH" }?.value ?? "unknown"
        var steps: [Step] = [Step(title: "portnanny on PATH", detail: onPath, kind: .note)]
        ...
        if has("Claude Code") {
            steps.append(Step(title: "Register the MCP server with Claude Code",
                              detail: "runs: claude mcp add --scope user portnanny -- portnanny mcp",
                              kind: .runCommand(["claude", "mcp", "add", "--scope", "user", "portnanny", "--", "portnanny", "mcp"])))
            steps.append(Step(title: "Add the PortNanny section to CLAUDE.md", detail: project.appendingPathComponent("CLAUDE.md").path, kind: .writeRules(.claude)))
            steps.append(Step(title: "The Claude Code plugin does all of that at once, plus the lsof hook",
                              detail: "claude plugin marketplace add mukes555/PortNanny && claude plugin install portnanny@portnanny", kind: .note))
        }
        if has("Codex CLI") {
            steps.append(Step(title: "Add the PortNanny section to AGENTS.md", detail: project.appendingPathComponent("AGENTS.md").path, kind: .writeRules(.codex)))
        ...
        if present.isEmpty {
            steps.append(Step(title: "No AI tool found on this Mac", detail: "install one, or write rules by hand: portnanny agent-docs --write", kind: .note))
        }
        let shellName = (shell as NSString).lastPathComponent
        let completions: String
        switch shellName {
        case "zsh": completions = "portnanny completions zsh > ~/.zfunc/_portnanny\n(with ~/.zfunc in fpath)"
        case "bash": completions = "portnanny completions bash >> ~/.bash_completion"
        case "fish": completions = "portnanny completions fish > ~/.config/fish/completions/portnanny.fish"
        default: completions = "portnanny completions zsh|bash|fish"
        }
