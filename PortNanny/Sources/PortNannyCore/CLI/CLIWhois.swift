...
/// `portnanny whois`: everything PortNanny knows about what is on a port,
...
public enum CLIWhois {
    ...
    public static func run(_ options: CLICommand.WhoisOptions) -> Int32 {
        ...
        if report.targets.isEmpty {
            PortNannyCLI.printError(text(for: report))
        } else {
    ...
    private static func ownerLines(_ dossier: Dossier) -> [String] {
        ...
        if let owner = dossier.agentOwner {
            let how = owner.source == .declared ? "declared via PORTNANNY_OWNER" : "from \(owner.source.rawValue)"
