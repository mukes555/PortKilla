...
/// Debug builds only. `portnanny __serve <port>` listens until killed, so the
/// scenario tests can spawn a real server whose environment markers and
/// ancestry PortNanny then has to attribute. macOS hides the environment of
...
enum DebugServer {
    static func serve(port: Int) -> Int32 {
        ...
        PortNannyCLI.printError("listening on 127.0.0.1:\(port)")
        while true {
            sleep(60)
        }
        #else
        PortNannyCLI.printError("portnanny: __serve is only available in debug builds")
