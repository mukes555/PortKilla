...
final class CLIArgumentsTests: XCTestCase {
    ...
    func testListFilters() {
        ...
        var options = CLICommand.ListOptions()
        options.mine = true
        XCTAssertEqual(PortNannyCLI.filtered(ports, by: options, caller: mine).map(\.port), [1])
        options = CLICommand.ListOptions(); options.unowned = true
        XCTAssertEqual(PortNannyCLI.filtered(ports, by: options, caller: nil).map(\.port), [4])
        options = CLICommand.ListOptions(); options.orphaned = true
        XCTAssertEqual(PortNannyCLI.filtered(ports, by: options, caller: nil).map(\.port), [3])
        options = CLICommand.ListOptions(); options.agent = "claude"
        XCTAssertEqual(PortNannyCLI.filtered(ports, by: options, caller: nil).map(\.port), [1, 2])
    }
}
