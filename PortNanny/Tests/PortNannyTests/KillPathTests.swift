...
final class KillPathTests: XCTestCase {
    ...
    func testURLCommandParsing() {
        ...
        XCTAssertEqual(URLCommand.parse(URL(string: "portnanny://kill/3000?force=1")!), .kill(port: 3000, force: true))
        XCTAssertEqual(URLCommand.parse(URL(string: "portnanny://kill/3000?force=0")!), .kill(port: 3000, force: false))
        XCTAssertEqual(URLCommand.parse(URL(string: "portnanny://show")!), .show)
        XCTAssertNil(URLCommand.parse(URL(string: "portnanny://kill/abc")!))
        XCTAssertNil(URLCommand.parse(URL(string: "portnanny://kill/70000")!))
        XCTAssertNil(URLCommand.parse(URL(string: "portnanny://reboot")!))
    }
}
