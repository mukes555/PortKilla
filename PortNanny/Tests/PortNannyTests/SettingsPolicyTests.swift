...
final class SettingsPolicyTests: XCTestCase {
    ...
    func testSetupStepsApplyAndReportInOneLine() throws {
        ...
        let write = CLISetup.Step(title: "Add the PortNanny section to CLAUDE.md", detail: "", kind: .writeRules(.claude))
        ...
        let note = CLISetup.Step(title: "Shell completions", detail: "portnanny completions zsh", kind: .note)
    ...
    func testABetaDownloadOpensItsOwnReleasePage() {
        XCTAssertEqual(UpdateChecker.releasePage(for: "2.1.0-beta.1").absoluteString, "https://github.com/mukes555/PortNanny/releases/tag/v2.1.0-beta.1")
    }
    ...
    func testTheSetupFolderMustExistAndNothingDefaultsToHome() throws {
        ...
        let file = FileManager.default.temporaryDirectory.appendingPathComponent("portnanny-not-a-folder-\(UUID().uuidString)")
    ...
    func testLeaseLengthsAlwaysIncludeTheCurrentChoice() {
        ...
        XCTAssertTrue(AgentsSettings.isCommand("portnanny completions zsh > ~/.zfunc/_portnanny"))
